...
for p_idx, protein in enumerate(hmmscan):
    ...
    for a,domain in enumerate(mapped_domains):
        ...
        if F_group in ecod_domain_dict.keys():
            ...
            domain_info[a] = [accession, str(eval), residue_range_as_string, ' '.join(str(s) for s in domain_properties), arch, x_group, t_group, F_group, f_id]
        else:
            domain_info[a] = [accession, str(eval), residue_range_as_string, ' '.join(str(s) for s in domain_properties), "N/A", "N/A", "N/A", F_group, "N/A"]
